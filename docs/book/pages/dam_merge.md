...
When updating a Resource, Apply *merges* the local Resource Config into the remote Resource.  This process will
ensure that Resource state owned by the cluster is not lost during an update.

## Shared Ownership of Fields

Resources may contain state owned by the the cluster that is not present in the Resource Config
specified by the user.  `clusterIp` for instance is set by the cluster and should not be
changed by the end user.  Some fields such as `replicas` may be owned by either the user or the cluster -
depending on whether the field is present in the Resource Config or not.
